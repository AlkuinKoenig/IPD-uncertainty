...
input_key_xlxs = paste0(here::here(),"/data_input/isotope_keys_testfile.xlsx")
#path to the excel sheet containing information about the samples (peak area, sample volume, etc) and uncertainties
input_data_xlxs = paste0(here::here(),"/data_input/sample_data_testfile.xlsx")
...
peakareas.uncert = tryCatch(expr={openxlsx::read.xlsx(input_data_xlxs, sheet = "peak area uncertainties abs",startRow=2)},
                       error=function(e){
...
for (k in 1:MC_iterations){
  ...
  data_trans.l[[k]] = rnorm(length(peakareas.m), mean = peakareas.m, sd = peakareas.uncert.m) %>%
...
write_output_individual = function(fname_out, df_individual.s, results.l){
  ...
  for (k in c("mean","uncert_k2","sd","median","q025","q975")){
...
if (write_deconvolution_output & write_individual_output){
  write_output_individual(fname_out = paste0(here::here(),"/output_data/MC_output_devonvolutions_individual_",timestamp,".xlsx"), 
                 results_deconvolution[[1]], deconvoluted.l)
}
if (write_concentration_output & write_individual_output){
  write_output_individual(fname_out = paste0(here::here(),"/output_data/MC_output_concentrations_individual_",timestamp,".xlsx"),
